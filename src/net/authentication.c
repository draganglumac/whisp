...
void authentication_update_foriegn_session(session *s) {
	...
	JNX_LOGC(JLOG_NORMAL,"<<<<<<<<< SENDING OUTGOING SESSION DATA [%s]\n",s->session_id);
...
state authentication_session_update(session *s) {
	...
	switch(s->current_state) {
		...
		case SESSION_PUBLIC_KEY_EXCHANGE:
			...
			JNX_LOGC(JLOG_NORMAL,"SESSION KEY EXCHANGE\n");
			break;
		case SESSION_HANDSHAKING:

			JNX_LOGC(JLOG_NORMAL,"Got public keys. Time to encrypt a shared session key\n");
			char *shared_key = session_generate_secret();

			JNX_LOGC(JLOG_NORMAL,"UNENCRYPTED SHARED KEY [%s]\n",shared_key);
			size_t olen;

			RSA *their_pubpair = string_to_key(s->foriegn_public_key,PUBLIC);
			JNX_LOGC(JLOG_NORMAL,"Using their foriegn key for encryption\n%s\n",key_to_string(their_pubpair,PUBLIC));
			char *encrypted = encrypt_message(their_pubpair,shared_key,&olen);

			JNX_LOGC(JLOG_NORMAL,"ENCRYPTED SHARED KEY [%dbbytes][%s]\n",olen,encrypted);
			...
			JNX_LOGC(JLOG_NORMAL,"SESSION HANDSHAKING\n");
		...
		case SESSION_CONNECTED:
			JNX_LOGC(JLOG_NORMAL,"SESSION CONNECTED\n");
			size_t len;

			if(strcmp(jnx_hash_get(configuration,"GUID"),s->session_origin_guid) == 0) {
				JNX_LOGC(JLOG_NORMAL,"Local peer connected with shared secret of %s\n",s->shared_secret);
				...
				JNX_LOGC(JLOG_NORMAL,"Foriegn peer connected with shared secret of %s\n",s->shared_secret);
				secure_channel_listener_resolve_with_session(s);
				free(shared_secret);
