...
char *session_generate_secret() {
    ...
    JNX_LOGF(JLOG_NORMAL,"SESSION SECRET: %s\n",buffer);
...
char *session_generate_id() {
    ...
    JNX_LOGF(JLOG_NORMAL,"SESSION ID: %s\n", buffer);
    assert(buffer);
    return strdup(buffer);
}
...
state session_get_state(char *session_id) {
    ...
    if(!s) {
        JNX_LOGF(JLOG_NORMAL,"Session not found\n");
        return SESSION_ERROR;
    }
...
int session_get_session(char *session_id, session **session_handle) {
    if(!session_tree) {
        JNX_LOGF(JLOG_NORMAL,"Session get session: no tree found\n");
        return 0;
    }
    JNX_LOGF(JLOG_NORMAL,"================Getting session [%s]===========\n",session_id);
    ...
    while(keys->head) {
        if(strcmp(keys->head->_data,session_id) == 0) {
            ...
            if(s) {
                JNX_LOGF(JLOG_NORMAL,"Found valid session %s\n",s->session_id);
                *session_handle = s;
...
char * session_create(raw_peer *local, raw_peer *foriegn) {
    if(!session_tree) {
        JNX_LOGF(JLOG_NORMAL,"Session create: no tree found; making\n");
...
void session_add(session *s) {
    if(!session_tree) {
        JNX_LOGF(JLOG_NORMAL,"SESSION ADD: No tree found, making...\n");
        session_tree = jnx_btree_create(sizeof(int),session_tree_compare);
    }
    int n = session_count();
    jnx_btree_add(session_tree,s->session_id,s);
    JNX_LOGF(JLOG_NORMAL,"==============Added to session tree [%s] Session Count %d============",s->session_id,session_count());
    assert(session_count() == (n+1));
}
void session_destroy(session *s) {
    assert(s);
    ///remove from the session tree
    JNX_LOGF(JLOG_NORMAL,"Removing from session tree\n");
    jnx_btree_remove(session_tree,s->session_id,NULL,NULL);
    JNX_LOGF(JLOG_NORMAL,"Deleting session id...\n");
    free(s->session_id);
    if(s->shared_secret) {
        JNX_LOGF(JLOG_NORMAL,"Deleting session shared secret...\n");
        free(s->shared_secret);
    }
    if(s->foriegn_public_key) {
        JNX_LOGF(JLOG_NORMAL,"Deleting session foriegn public key...\n");
        free(s->foriegn_public_key);
    }
    if(s->local_public_key) {
        JNX_LOGF(JLOG_NORMAL,"Deleting session public key...\n");
        free(s->local_public_key);
    }
    JNX_LOGF(JLOG_NORMAL,"Deleting session object...\n");
    JNX_MEM_FREE(s);
}
int session_check_exists(raw_peer *local, raw_peer *foriegn) {
    if(!session_tree) {
        JNX_LOGF(JLOG_NORMAL,"Session check exists : no tree found, making\n");
        session_tree = jnx_btree_create(sizeof(int),session_tree_compare);
    }
    jnx_list *keys = jnx_list_create();
    jnx_btree_keys(session_tree,keys);
...
int session_check_exists_by_id(char *session_id) {
    if(!session_tree) {
        JNX_LOGF(JLOG_NORMAL,"BUILDING SESSION TREE\n");
        session_tree = jnx_btree_create(sizeof(int),session_tree_compare);
        return 0;
    }
    jnx_list *keys = jnx_list_create();
...
int session_count() {
    if(!session_tree) {
        JNX_LOGF(JLOG_NORMAL,"session_count: no session tree\n");
        return 0;
    }
    int n = 0;
